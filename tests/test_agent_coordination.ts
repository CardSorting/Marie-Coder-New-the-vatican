...
function createYOLODecision(
    strategy: CouncilStrategy = 'EXECUTE',
    confidence: number = 2.0,
...
async function testCircularDependencyDetection() {
    ...
    assert.ok(conflicts.some(c => c.issue.includes('Circular dependency')),
        'Should detect circular dependency');
    assert.ok(conflicts.some(c => c.severity === 'CRITICAL'),
        'Circular dependency should be CRITICAL severity');
...
async function testYOLOPrioritization() {
    ...
    assert.ok(result.recommendations.some(r => r.includes('YOLO guidance')),
        'Should include YOLO guidance in recommendations');
...
async function testYOLOInfluenceAnalytics() {
    ...
    assert.ok(['INCREASING', 'DECREASING', 'STABLE'].includes(analytics.trendDirection),
