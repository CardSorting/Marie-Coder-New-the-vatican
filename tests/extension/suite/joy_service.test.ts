...
suite('JoyService Mock Tests', () => {
    ...
    test('Initializes with a status bar item', () => {
        assert.ok(context.subscriptions.length > 0, 'Should have registered status bar item in subscriptions');
    });
    ...
    test('addAchievement adds to log', async () => {
        ...
        assert.ok(log.some(a => a.description === 'Test Achievement'), 'Achievement should be in log');
    });
