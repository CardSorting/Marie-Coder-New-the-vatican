...
async function testSelfHealing() {
    ...
    const result = await automationService.executeSelfHealing('path/to/fail.ts', 'content check failed');

    assert.ok(result.includes('Self-healing'), 'Should return self-healing message');
