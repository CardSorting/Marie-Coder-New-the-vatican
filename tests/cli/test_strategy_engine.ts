...
class MockAIProvider implements AIProvider {
    ...
    async createMessageStream(params: any, onUpdate: (e: AIStreamEvent) => void): Promise<any> {
        const scenario = this.responseFlow[this.currentIndex] || { events: [], response: { role: "assistant", content: "Done" } };
        this.currentIndex++;
...
async function testCliEnvironment() {
    ...
    const tracker = new MarieProgressTracker({
        onEvent: (e) => {
            events.push(e);
            if (e.type === 'stage') console.log(`[Test Debug] Stage change: ${e.stage}`);
            if (e.type === 'progress_update') console.log(`[Test Debug] Progress: ${e.activeObjectiveId} -> ${e.context}`);
        },
    }, run);
    ...
    const engine = new MarieEngine(provider, toolRegistry, async () => true, undefined, fs);
    ...
    console.log(`[Test Debug] Event Types: ${events.map(e => e.type).join(', ')}`);

    assert.strictEqual(state.environment, 'cli', 'Environment should be detected as cli');
    assert.strictEqual(run.activeObjectiveId, 'execute_plan', 'Objective should transition to execute_plan');
    assert.ok(events.some(e => e.type === 'tool_delta'), 'Should have emitted tool_delta');
...
async function testSequentialTools() {
    ...
    provider.setResponseFlow([
        {
            ...
            response: { role: 'assistant', content: '', tool_uses: [{ id: 't1', name: 'tool_1', input: {} }] }
        },
        {
            events: [
            ...
            response: { role: 'assistant', content: '', tool_uses: [{ id: 't2', name: 'tool_2', input: {} }] }
        },
        {
    ...
    await engine.chatLoop([], tracker, async () => { });
    assert.strictEqual(run.activeObjectiveId, 'execute_plan', 'Should remain in execute_plan after first tool');

    await engine.chatLoop([], tracker, async () => { });
    assert.strictEqual(run.activeObjectiveId, 'execute_plan', 'Should remain in execute_plan after second tool');
...
async function testToolError() {
    ...
    provider.setResponseFlow([
        {
            ...
            response: { role: 'assistant', content: '', tool_uses: [{ id: 't1', name: 'buggy_tool', input: {} }] }
        },
        {
    ...
    assert.strictEqual(run.activeObjectiveId, 'execute_plan', 'Should transition even if tool fails');
