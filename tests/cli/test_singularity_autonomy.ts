...
async function testSingularityAutonomy() {
    ...
    const tracker = new MarieProgressTracker({} as any, { runId: 'test', totalPasses: 3 } as any);
    const processor = new MarieToolProcessor({} as any, tracker, async () => true, {} as any);

    // Mock getDiagnostics to return an error
    (vscodeMock.languages as any).getDiagnostics = () => [
        { severity: 0, message: 'Syntax error: Unexpected token', range: { start: { line: 10 } } }
    ];

    const buildAlert = await (processor as any).runBuildSentinel('test.ts');
    assert.ok(buildAlert.includes('Build Regressions Detected'), 'Build Sentinel should detect errors');
