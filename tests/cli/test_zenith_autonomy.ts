...
async function testZenithAutonomy() {
    ...
    const tracker = new MarieProgressTracker({} as any, {
        runId: 'test_run',
        startedAt: Date.now(),
        totalPasses: 3,
        currentPass: 1,
    ...
    assert.strictEqual(tracker.getRun().totalPasses, 4, 'Total passes should autonomously increment');
    ...
    const criticalFile = '/Users/bozoegg/Downloads/Marie-Coder-New-the-vatican/src/monolith/domain/ZenithCore.ts';
    ...
    const anchorEvent = events.find((e: any) => e.text && e.text.includes('⚓ ZENITH'));
