# 📔 MarieCoder Project Documentation

Welcome to the heart of the MarieCoder project. This garden of knowledge is designed to help you understand the philosophy, architecture, and machinery behind your highly capable AI coding partner.

---

## 🗺️ Navigation Portal

### 🚀 Getting Started
- **[Setup Guide](SETUP.md)**: How to prepare your environment and plant the seeds of the project.
- **[Contributing](CONTRIBUTING.md)**: How to nurture the garden together.
- **[Troubleshooting](TROUBLESHOOTING.md)**: What to do when things don't spark joy.

### 🏗️ Core Components
- **[ARCHITECTURE.md](file:///Users/bozoegg/Downloads/Marie-Coder-New-the-vatican/docs/ARCHITECTURE.md)**: The structural law.
- **[NAVIGATION_ARCHITECTURE.md](file:///Users/bozoegg/Downloads/Marie-Coder-New-the-vatican/docs/NAVIGATION_ARCHITECTURE.md)**: Senior-level URL and routing strategy.
- **[AGENT_STREAM_CONTROL_PLANE.md](file:///Users/bozoegg/Downloads/Marie-Coder-New-the-vatican/docs/AGENT_STREAM_CONTROL_PLANE.md)**: How agents think and move.
- **[Rituals & Memory](RITUALS_AND_MEMORY.md)**: Strategic context and gratefulness rituals.
- **[Tidying & Health](TIDYING_GUIDE.md)**: Joy scores and the Tidying Checklist.
- **[Living Project Lifecycle](LIVING_PROJECT.md)**: Understanding Sprouts, Blooms, and Compost.

### 🛠️ Capabilities & Machinery
- **[Marie's Toolset](TOOLS.md)**: A comprehensive, clickable guide to all autonomous tools.
- **[Recovery & Strategies](RECOVERY_STRATEGIES.md)**: How Marie handles failure with Sovereign Recovery and Reflection.

### 🧘 Rituals & Standards
- **[KonMari Standards](konmari-standards.md)**: The philosophy of mindfulness and naming.
- **[Strategic Planning Ritual](ARCHITECTURE.md#strategic-planning-ritual)**: The mandatory steps before any complex task.

---

## 🌸 Philosophy of Documentation
Our documentation is a living thing. We prioritize **Intent** over **Mechanism**. If a document no longer sparks joy or clarity, it should be composted or bloomed into something better.

> "A document is a gift to your future self." ✨
