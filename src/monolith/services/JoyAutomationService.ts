...
import { proposeReorganization, executeRestoration, synthesizeZoneManuals, scaffoldZoneAbstractions, sowFeature, proposeClustering, executeGenesisRitual, isProjectJoyful, generateJoyDashboard, generateTidyChecklist, ensureJoyZoningFolders } from '../domain/joy/JoyTools.js';
...
export class JoyAutomationService {
    ...
    public async performGardenPulse(): Promise<string> {
        ...
        try {
            ...
            if (proposals.length > 0) {
                ...
                const { ConfigService } = await import("../infrastructure/config/ConfigService.js");
                const autoRepair = ConfigService.getAutoRepair();

                if (proposals.length > 5) {
                    if (autoRepair) {
                        await this.joyService.addAchievement("Auto-Repair initiated for significant drift. 🛠️", 10);
                        const repairResult = await this.executeAutonomousRestoration();
                        finalReport += `\n- 🛠️ **Auto-Repair Executed**: ${repairResult}`;
                    } else {
                        vscode.window.showWarningMessage(`⚠️ Joy Heartbeat: Significant structural drift (${proposals.length} items) detected. Consider a restoration ritual.`);
                    }
                }
            }
    ...
    public startAutonomousHeartbeat(intervalMs: number = 300000) { // Default 5 mins
    ...
    public async executeSelfHealing(failedPath: string, errorReason: string): Promise<string> {
        ...
        if (errorReason.includes('content check failed') || errorReason.includes('could not find')) {
    ...
    private async performSpiritualJournaling() {
        ...
        try {
            ...
            if (!stats) {
                const header = "# Spiritual Journal\n\nA record of the project's architectural evolution and the AI's reflections.\n\n";
                await vscode.workspace.fs.writeFile(vscode.Uri.file(journalPath), Buffer.from(header + entry, 'utf8'));
            } else {
                const existing = await vscode.workspace.fs.readFile(vscode.Uri.file(journalPath));
                await vscode.workspace.fs.writeFile(vscode.Uri.file(journalPath), Buffer.from(existing.toString() + entry, 'utf8'));
            }
        } catch (e) {
    ...
    private async scoutAchievements() {
        ...
        try {
            ...
            await this.joyService.addAchievement("The Garden Pulse reached perfect resonance. Singularity is near. 🌌", 5);

            // Achievement: "Zenith Master" - All 3 JOY zones exist and are scaffolded
            const joyful = await isProjectJoyful(root);
            if (joyful) {
                await this.joyService.addAchievement("Structural Harmony attained. All JOY zones are flourishing. 🏛️", 20);
            }
        } catch (e) {
